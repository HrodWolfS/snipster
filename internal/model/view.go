...
func (m Model) viewConfirmDelete() string {
    name := ""
    if m.editing != nil { name = m.editing.Title }
...
func (m Model) headerStatus() string {
    ...
    count := 0
    for _, it := range m.VisibleItems { if it.Kind == SidebarItemSnippet && it.Snippet != nil { count++ } }
    ...
    if m.Status != "" { parts = append(parts, m.Status) }
